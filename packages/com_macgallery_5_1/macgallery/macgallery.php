...
class plgSystemMacgallery extends JPlugin
{
	var $config;
	var $subject;
	var $params;
	...
	public function __construct( &$subject, $config )
	{
			...
			$this->config = $config;
			$plugin =& JPluginHelper::getPlugin('system', 'macgallery');
			//$this->params = new JParameter( $plugin->params );
			// define the regular expression for the bot
			$this->regex = "#(<p\b[^>]*>\s*)?{mac(\s)+albumid=['\"(](.*?)['\")](\s)+rows=['\"(](.*?)['\")]}(\s*</p>)?#s";
	...
	function _replace(&$text) {
		...
		for($counter = 0; $counter < $cnt; $counter ++) {
			$content = $this->_process($matches[2][$counter][0],$counter,$matches[0][$counter][0]);
			$text = preg_replace($this->regex, $content, $text, 1);
		}
	}
	
	function _process( $url,$counter,$searchText="") {
		// Clean url
		preg_replace('/\s\s+/', ' ',$searchText);
		preg_match_all('/(?P<name>\w+)=\'(?P<digit>\d+)\'/', $searchText, $searchmatches);
		
		$plgCols  = (isset($searchmatches["digit"][2]))? $searchmatches["digit"][2] : 0;
		$plgRows  = (isset($searchmatches["digit"][1]))? $searchmatches["digit"][1] : 0;
		$albumId =  (isset($searchmatches["digit"][0]))? $searchmatches["digit"][0] : 0; 
		...
		$row = preg_replace( $reg, $repl, trim($url) );
		
		
		if (strpos($url, 'index.php')!== false||$this->ignoresef=="1") {
			$sef=false;
			$url = "index.php?option=com_macgallery&view=images&albumid=".$albumId;
			...
			$url = "component/macgallery/images.html";
			// Add origin too to the component so it can redirect to the origin if something goes wrong		
			//$url .= '/origin='.base64_encode( JUri::getInstance()->toString() );
		}
	
		$url = "index.php?option=com_macgallery&view=images&albumid=".$albumId;
		$url .= "&tmpl=component&format=raw&albumLoad=0&plg=1&counter=".$counter."&plgrows=".$plgRows."&plgCols=".$plgCols."&";
