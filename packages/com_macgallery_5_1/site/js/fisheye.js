	...
	build : function(options)
	{
		...
		return this.each(
			function()
			{
				var el = this;
				el.fisheyeCfg = {
					items : appthaJQuery(options.items, this),
					container: appthaJQuery(options.container, this),
					pos : appthaJQuery.iUtil.getPosition(this),
				...
				appthaJQuery.iFisheye.positionContainer(el, 0);
				appthaJQuery(window).bind(
					'resize',
					function()
					{
						el.fisheyeCfg.pos = appthaJQuery.iUtil.getPosition(el);
						appthaJQuery.iFisheye.positionContainer(el, 0);
						appthaJQuery.iFisheye.positionItems(el);
					}
				);
				appthaJQuery.iFisheye.positionItems(el);
				el.fisheyeCfg.items
					.bind(
						...
						function()
						{
							appthaJQuery(el.fisheyeCfg.itemsText, this).get(0).style.display = 'none';
						}
					)
					.bind(
						'mouseout',
				...
				appthaJQuery(document).bind(
					'mousemove',
					function(e)
					{
						var pointer = appthaJQuery.iUtil.getPointer(e);
						var toAdd = 0;
						...
						appthaJQuery.iFisheye.positionContainer(el, toAdd);
					}
				);
			}
		)
...
appthaJQuery.fn.Fisheye = appthaJQuery.iFisheye.build;